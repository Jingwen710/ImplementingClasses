...
class Point(object):
    ...
    def move_to(self, x, y):
        self.distance = self.distance + ((x - self.x) ** 2 + (y - self.y) ** 2) ** (1 / 2)
        self.x = x
        self.y = y
    ...
    def get_distance_from_start(self):
        distance = ((self.x - self.xx)** 2 + (self.y - self.yy) ** 2) ** (1/2)
        return distance
    ...
    def closer_to(self, p2, p3):
        if self.get_distance_from(p2) < self.get_distance_from(p3):
            return p2
        elif self.get_distance_from(p2) == self.get_distance_from(p3):
            return p2
        else:
            return p3
